...
export async function discoverGreenhouseCompanies(
  ...
  log(`🎯 Testing ${potentialCompanies.length} potential companies from curated lists\n`, 'info')
  ...
  for (const slug of companySlugsToTest) {
    ...
    if (tested === 1 || tested % 5 === 0 || tested === total) {
      log(`   Progress: ${tested}/${total} (${((tested/total)*100).toFixed(1)}%)`, 'info')
    }
  ...
  log(`🎯 Success rate: ${tested > 0 ? ((discovered.length / tested) * 100).toFixed(1) : 0}%`, 'info')

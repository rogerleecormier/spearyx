...
import { createSync, addSyncLog, completeSync, getSync, subscribeToSync, unsubscribeFromSync } from '../../lib/jobs/sync-state'
import { createSyncReport, updateDiscoveryStats, updateJobStats, finalizeSyncReport, formatSyncReport, parseDiscoveryOutput, parseJobSyncOutput, addTopJob } from '../../lib/jobs/sync-report'
    ...
    handlers: {
      GET: async ({ request, context }) => {
        ...
        try {
          ...
          return new Response(JSON.stringify({ error: 'Database connection failed', details: String(error) }), {
            status: 500,
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            try {
              ...
              sendLog('🚀 Starting job sync process (Cloudflare D1 Mode)...', 'info')
              sendLog('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'info')
              sendLog('', 'info')
              ...
              if (discovery) {
                ...
                try {
                  ...
                  const ghStats = await discoverGreenhouseCompanies(db, sendLog, request.signal)
                  ...
                  sendLog('⚠️ Lever Discovery not yet migrated to D1', 'warning')
              ...
              if (maintenance) {
                ...
                sendLog('⚠️ Maintenance scripts not yet migrated to D1', 'warning')
                sendLog('', 'info')
              }

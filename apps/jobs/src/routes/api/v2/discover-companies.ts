  ...
  server: {
    handlers: {
      POST: async ({ request, context }) => {
        try {
          console.log('🔍 Company discovery started');
          ...
          console.log(`📊 Checking companies ${startIndex}-${endIndex-1} of ${potentialCompanies.length}`);
          ...
          for (const company of companiesToCheck) {
            console.log(`🔎 Checking: ${company.slug}`);
            ...
            if (result.found && result.company) {
              ...
              try {
                ...
                console.log(`✅ Discovered: ${company.slug} (${result.company.remoteJobCount} remote jobs)`);
              } catch (error: any) {
                if (error.message?.includes('UNIQUE constraint')) {
                  ...
                  console.log(`ℹ️  ${company.slug} already in discovered companies`);
                } else {
                  throw error;
                }
              }

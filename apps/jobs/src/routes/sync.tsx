...
function SyncPage() {
  ...
  const selectPreset = (preset: string) => {
    const presets: Record<string, string[]> = {
      ...
      discovery: SCRIPTS.filter((s) => s.category === "discovery").map(
        (s) => s.id
      ),
  ...
  const runScripts = async () => {
    ...
    const hasDiscovery = SCRIPTS.some(
      (s) => s.category === "discovery" && selectedScripts.has(s.id)
    );
    const hasCleanup = SCRIPTS.some(
      (s) => s.category === "cleanup" && selectedScripts.has(s.id)
    );
    const hasMaintenance = SCRIPTS.some(
      (s) => s.category === "maintenance" && selectedScripts.has(s.id)
    );
    const hasSources = SCRIPTS.some(
      (s) => s.category === "sources" && selectedScripts.has(s.id)
    );
    ...
    const selectedSources = SCRIPTS.filter(
      (s) => s.category === "sources" && selectedScripts.has(s.id)
    ).map((s) => s.name);
        ...
        <div>
          ...
          >
            ...
            >
              ...
              <button onClick={selectNone} style={buttonStyle}>
                Select None
              </button>
            </div>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                ...
                <button
                  onClick={() => selectPreset("discovery")}
                  style={presetButtonStyle}
                >
                  Discovery Only
                </button>
                <button
                  onClick={() => selectPreset("cleanup")}
                  style={presetButtonStyle}
                >
                  Cleanup Only
                </button>
            ...
            >
              <button
                ...
                style={{
                  ...
                  cursor:
                    isRunning || selectedScripts.size === 0
                      ? "not-allowed"
                      : "pointer",
                }}
              >
                {isRunning ? (
                  <>
                    <Loader2 size={18} className="jobs-spinner" />
                    Running...
                  </>
                ) : (
                  <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {syncReport && (
            ...
            >
              ...
              >
                <StatCard
                  label="Companies Tested"
                  value={syncReport.discovery?.companiesTested || 0}
                />
                <StatCard
                  label="Companies Discovered"
                  value={syncReport.discovery?.companiesDiscovered || 0}
                  color="#10b981"
                />
                <StatCard
                  label="Jobs Fetched"
                  value={syncReport.jobs?.totalFetched || 0}
                  color="#3b82f6"
                />
              </div>
              ...
              >
                <StatCard
                  label="Jobs Added"
                  value={syncReport.jobs?.added || 0}
                  color="#10b981"
                />
                <StatCard
                  label="Jobs Updated"
                  value={syncReport.jobs?.updated || 0}
                  color="#f59e0b"
                />
                <StatCard
                  label="Jobs Skipped"
                  value={syncReport.jobs?.skipped || 0}
                  color="#64748b"
                />
              </div>

/**
 * Aggregator Sync Worker (V3) - Self-Healing
 * Calls /api/v3/sync/aggregator with automatic retry on failure
 */
...
export default {
  async scheduled(_event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    console.log(`[${timeStr}] 🌐 Aggregator Sync Worker starting`);
    
    const url = `https://jobs.spearyx.com/api/v3/sync/aggregator`;
    ...
    try {
      ...
      if (result.success) {
        console.log(`[${timeStr}] ✅ ${result.source}: +${result.jobsAdded} added, ${result.jobsUpdated} updated (${result.duration}ms)`);
      } else {
        console.log(`[${timeStr}] ⚠️ Aggregator sync returned: ${result.error || 'no error message'}`);
      }
    } catch (error) {
      console.error(`[${timeStr}] ❌ Aggregator worker error after ${MAX_RETRIES} retries:`, error);
    }
  }
}
